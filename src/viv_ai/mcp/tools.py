...
def build_tool_metadata() -> Dict[str, Dict[str, Any]]:
    ...
    return {
        ...
        'get_dangerous_sinks': {
            'description': 'Return a list of dangerous sink function addresses and names for the workspace.',
            'inputSchema': _schema({
                'workspace_id': workspace_id,
        ...
        'get_attacker_sources': {
            'description': 'Return a list of attacker-controlled source function addresses and names for the workspace.',
            'inputSchema': _schema({
                'workspace_id': workspace_id,
                'max_results': max_items,
            }, ['workspace_id']),
        ...
        'find_attack_paths': {
            'description': 'Find data-flow paths from attacker sources to dangerous sinks.',
            'inputSchema': _schema({
                'workspace_id': workspace_id,
                'sink_addresses': {'type': 'array', 'items': {'type': 'string'}, 'description': 'Optional explicit sink addresses. Defaults to all dangerous sinks.'},
                'source_addresses': {'type': 'array', 'items': {'type': 'string'}, 'description': 'Optional explicit source addresses. Defaults to all attacker sources.'},
                'max_depth': {'type': 'integer', 'minimum': 1, 'default': 32, 'description': 'Maximum call depth for path discovery.'},
            }, ['workspace_id']),
            'annotations': {'readOnlyHint': True},
        },
    }
